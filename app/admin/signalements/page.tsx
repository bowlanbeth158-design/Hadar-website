...
const STATUS_CONFIG: Record<
  Status,
  { label: string; pill: string; text: string; Icon: LucideIcon; gradient: string; glow: string; progressBar: string }
> = {
  en_cours: {
    label: 'En cours',
    pill: 'bg-orange-100',
  ...
  a_corriger: {
    ...
    Icon: PencilLine,
    gradient: 'bg-grad-stat-navy',
    glow: 'shadow-glow-navy',
    progressBar: 'bg-brand-navy',
  },
};
    ...
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-8">
        {KPIS.map((k) => {
          ...
          return (
            <div
              key={k.status}
              className={`${c.gradient} ${c.glow} text-white rounded-2xl overflow-hidden shadow-sm`}
            >
      ...
      <section className="rounded-2xl bg-white border border-gray-200 shadow-glow-soft overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100 text-xs text-gray-500">
          <span>5 signalements sur la période sélectionnée</span>
          <div className="flex items-center gap-1">
            <button type="button" className="px-2 py-1 rounded hover:bg-gray-100" disabled>
              ‹
            </button>
            <button type="button" className="px-2.5 py-1 rounded bg-brand-navy text-white font-semibold">
              1
            </button>
            <button type="button" className="px-2 py-1 rounded hover:bg-gray-100" disabled>
              ›
