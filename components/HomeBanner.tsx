    ...
    <section className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-[1440px] px-6 md:px-10 pt-4 md:pt-6 pb-12 md:pb-20 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <div>
          ...
          {/* CTAs — desktop keeps the natural left-aligned wrap.
              On phone (max-md) the row centers itself + each
              button gets max-md:flex-1 so they share the width
              equally, and max-md:justify-center keeps icon+label
              centered inside the now-stretched pill. */}
          <div className="mt-7 flex flex-wrap items-center gap-3 max-md:justify-center">
            <VerifyNowCta className="max-md:flex-1 max-md:justify-center" />
            <Link
              href="/signaler"
          ...
          {/* MOBILE-ONLY visual block — desktop keeps the original
              right-column layout untouched (hidden lg:block). The
              phone version mirrors the desktop FLOATING layout
              (cards orbit the ambassador photo) but tuned to fit
              a phone width: smaller container (max-w-[20rem]),
              slimmer cards (w-44 / w-40), tighter offsets so they
              hug the photo instead of extending far past it.
          ...
          <div className="lg:hidden mt-10">
            <div className="relative mx-auto w-full max-w-[24rem] aspect-[4/5]">
              ...
              <img
                ...
                alt=""
                aria-hidden
                className="absolute inset-0 h-full w-full object-contain object-top scale-110 origin-top blur-md opacity-80 [mask-image:linear-gradient(to_bottom,transparent_30%,rgba(0,0,0,0.75)_50%,rgba(0,0,0,0.35)_68%,transparent_88%)] [-webkit-mask-image:linear-gradient(to_bottom,transparent_30%,rgba(0,0,0,0.75)_50%,rgba(0,0,0,0.35)_68%,transparent_88%)]"
              />
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={AMBASSADOR_IMAGE_URL}
                alt=""
                aria-hidden
                className="absolute inset-0 h-full w-full object-contain object-top scale-110 origin-top [mask-image:linear-gradient(to_bottom,black_28%,rgba(0,0,0,0.75)_48%,rgba(0,0,0,0.4)_64%,rgba(0,0,0,0.15)_78%,transparent_92%)] [-webkit-mask-image:linear-gradient(to_bottom,black_28%,rgba(0,0,0,0.75)_48%,rgba(0,0,0,0.4)_64%,rgba(0,0,0,0.15)_78%,transparent_92%)]"
              />

              {/* Card 1 — Recherche instantanée (mid-left, slightly
                  outside the container so it visually "floats" past
                  the photo edge like the desktop layout). */}
              <div
                className={`absolute top-[42%] -left-3 rtl:left-auto rtl:-right-3 w-44 rounded-2xl border border-white/80 bg-gradient-to-br from-brand-sky/80 via-white to-brand-sky/30 shadow-glow-soft p-2.5 animate-float-soft ${FLOAT_CARD_HOVER}`}
                style={{ animationDelay: '0s' }}
              >
                <div className="flex items-center gap-1.5">
                  <Sparkles
                    className="h-4 w-4 text-brand-blue drop-shadow-sm animate-sparkle-pop"
                    aria-hidden
                  />
                ...
                <p className="mt-0.5 text-[10px] text-gray-500 leading-tight line-clamp-2">
                  {t('home.banner.card1.subtitle')}
                </p>
                <div
                  aria-hidden
                  className="mt-2 flex items-center gap-1.5 rounded-pill bg-white/90 backdrop-blur-sm border border-white shadow-sm px-2 py-1.5"
                >
                  <span className="flex-1 text-[10px] text-gray-400 truncate">
                    {t('home.banner.card1.placeholder')}
                  </span>
                  <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-gradient-to-br from-brand-navy to-brand-blue text-white shadow shrink-0">
                    <ArrowUp className="h-3 w-3" aria-hidden />
                  </span>
                </div>
              </div>

              {/* Card 2 — Utilisateurs rassurés (top-right). */}
              <div
                className={`absolute top-1 -right-2 rtl:right-auto rtl:-left-2 w-40 rounded-2xl border border-white/80 bg-gradient-to-br from-white via-brand-sky/40 to-brand-sky/70 shadow-glow-soft p-2.5 animate-float-soft ${FLOAT_CARD_HOVER}`}
              ...
              >
                <div className="flex items-center justify-between gap-1.5">
                  ...
                  <div className="inline-flex items-center gap-0.5 animate-stars-flash">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star
                        key={i}
                        className="h-2.5 w-2.5 text-yellow-300 fill-yellow-300 animate-star-pop"
                ...
                <p className="mt-1.5 text-[11px] font-bold text-brand-navy leading-tight">
              ...
              {/* Card 3 — Alertes aujourd'hui (bottom-right). */}
              <div
                className={`absolute bottom-2 -right-3 rtl:right-auto rtl:-left-3 w-44 overflow-hidden rounded-2xl bg-white border border-gray-200 shadow-glow-soft p-2.5 animate-float-soft ${FLOAT_CARD_HOVER}`}
                style={{ animationDelay: '3s' }}
              >
                <div
                  aria-hidden
                  className={`pointer-events-none absolute -bottom-10 -right-10 rtl:right-auto rtl:-left-10 h-28 w-28 rounded-full blur-3xl animate-pulse ${haloClass}`}
                />
                <div className="relative">
                  <div className="pb-1.5 border-b border-gray-100 flex items-center gap-1">
                    <BellRing
                  ...
                  <ul className="mt-1.5 space-y-1.5 text-[10px]">
                    <li className="flex items-center justify-between">
                      <span className="inline-flex items-center gap-1.5 font-semibold text-brand-navy">
                        <span className="h-2 w-2 rounded-full bg-yellow-300 ring-2 ring-yellow-100" />
          ...
          <p className="mt-8 md:mt-10 text-[13px] sm:text-sm md:text-base text-gray-500 whitespace-nowrap max-md:text-center">
