...
export function Header() {
  return (
    <>
      {/* Top WhatsApp strip — same container + invisible logo spacer so "Rejoignez"
          starts at the exact same X as "Accueil" in the nav below */}
      <div className="w-full bg-brand-navy text-white text-xs md:text-sm py-2">
        <div className="mx-auto max-w-7xl flex items-center gap-10 px-6 md:px-10">
      ...
      <header className="sticky top-0 z-40 w-full bg-white/90 backdrop-blur border-b border-gray-200">
        <nav className="mx-auto max-w-7xl flex items-center justify-between gap-6 px-6 md:px-10 py-3">
          {/* LEFT CLUSTER : logo + nav (nav starts right after the logo, same gap as the
              invisible spacer in the top banner -> "Accueil" aligns with the "R" of "Rejoignez") */}
